...
const Footer = () => {
  return (
    <section id='sec' className={`${styles.flexCenter} ${styles.paddingY} flex-col `}>
      ...
      <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
        ...
        <div className='flex flex-row md:mt-0 justify-center mt-6'>
          {
            socialMedia.map((social, index)=>(
              <img src={social.icon} key={social.id} alt={social.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? 'mr-4': 'mr-0'}`}/>
            ))
          }
        </div>
      </div>
